...
if (isset($_POST['reg_user'])) {
  ...
  if (empty($username)) { array_push($errors, "Tên người dùng bắt buộc"); }
  if (empty($email)) { array_push($errors, "Email bắt buộc"); }
  if (empty($password_1)) { array_push($errors, "Mật khẩu bắt buộc"); }
...
if (isset($_POST['login_user'])) {
  ...
  if (empty($password)) {
  	array_push($errors, "Mật khẩu bắt buộc");
  }
  ...
  if (count($errors) == 0) {
  	...
  	if (mysqli_num_rows($results) == 1) {
  	  ...
  	  header('location: index.php');
  		...
  		array_push($errors, "Tên người dùng hoặc mật khẩu sai");
  	}
  }
}
